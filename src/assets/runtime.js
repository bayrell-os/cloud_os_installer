...
Object.assign(Runtime.rtl,
{
	...
	m_to: function(ctx, type_value, def_value)
	{
		...
		return (ctx, m) => 
		{
			return new Runtime.Monad(ctx, (m.err == null) ? (this.convert(ctx, m.val, type_value, def_value)) : (def_value));
		};
	},
	/**
...
Object.assign(Runtime.RuntimeUtils,
{
	...
	{
		var __memorize_value = Runtime.rtl._memorizeValue("Runtime.RuntimeUtils.getParents", arguments);
		...
		Runtime.rtl._memorizeSave("Runtime.RuntimeUtils.getParents", arguments, __memorize_value);
	...
	{
		var __memorize_value = Runtime.rtl._memorizeValue("Runtime.RuntimeUtils.getFieldInfo", arguments);
		if (__memorize_value != Runtime.rtl._memorize_not_found) return __memorize_value;
		...
		for (var i = 0;i < parents.count(ctx);i++)
		{
			...
			var getFieldInfoByName = Runtime.rtl.method(ctx, parent_class_name, "getFieldInfoByName");
			var res = getFieldInfoByName(ctx, field_name);
			if (res != null)
			{
				var __memorize_value = res;
				Runtime.rtl._memorizeSave("Runtime.RuntimeUtils.getFieldInfo", arguments, __memorize_value);
				return __memorize_value;
			}
		}
		var __memorize_value = null;
...
Object.assign(Runtime.Core.Context,
{
	...
	create: function(ctx, env)
	{
		...
		/* Context data */
		var obj = Runtime.Dict.from({"enviroments":env,"settings":settings,"modules":Runtime.Collection.from([]),"object_manager":new Runtime.Core.ObjectManager(ctx, "Runtime.Core.ObjectManager")});
		/* Create context */
...
Runtime.Core.ObjectManager = function(ctx, object_name, entity)
{
	if (object_name == undefined) object_name = "";
	if (entity == undefined) entity = null;
	Runtime.Core.CoreDriver.call(this, ctx, object_name, entity);
	/* Create object */
...
Object.assign(Runtime.Web.Component,
{
	...
	getFieldInfoByName: function(ctx,field_name)
	{
		...
		if (field_name == "controller") return new IntrospectionInfo(ctx, {
			"kind": IntrospectionInfo.ITEM_FIELD,
			"class_name": "Runtime.Web.Component",
			"t": "Runtime.Web.RenderController",
			"name": field_name,
			"annotations": Collection.from([
			]),
		});
...
Object.assign(Runtime.Web.RenderController.prototype,
{
	 ...
	 */
	updateComponent: function(ctx, control, component, created)
	{
		if (created == undefined) created = false;
		this.updated_components.push(ctx, Runtime.Dict.from({"control":control,"component":component,"created":created}));
	},
	/**
	...
	e: function (control, childs, type, obj)
	{
		...
		if (type == 'component')
		{
			...
			*/
			...
			{
				/* Render component */
				var res = component.repaint(ctx, controller.layout, model, dict_attrs, content, new_control, created);
			...
			controller.updateComponent(ctx, new_control, component, created);
...
Object.assign(Runtime.Web.Route,
{
	...
	_initData: function(ctx, old, changed)
	{
		var uri = this._initDataGet(ctx, old, changed, "uri");
		var uri_match = this._initDataGet(ctx, old, changed, "uri_match");
		if (uri_match == "")
		{
			var uri_match = uri;
			uri_match = Runtime.re.replace(ctx, "\\/", "\\/", uri_match);
			var params = new Runtime.Vector(ctx);
			var matches = Runtime.re.matchAll(ctx, "{(.*?)}", uri);
			if (matches)
			{
				...
				changed = changed.setIm(ctx, "params", params);
			}
			else
			{
				changed = changed.setIm(ctx, "params", Runtime.Collection.from([]));
			}
			changed = changed.setIm(ctx, "uri_match", "^" + Runtime.rtl.toStr(uri_match) + Runtime.rtl.toStr("$"));
		}
		return changed;
	},
	/**
